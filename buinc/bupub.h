...
BUPUB_API int SetCol(void *handle,...);
...
// 参数  : char *rtnMsg
BUPUB_API int PutRow(void *handle,ST_PACK *pdata,int *pRetCode,char *rtnMsg);
...
BUPUB_API int IntCall(char *szBUGroupID, int waittime, ST_CPACK *rpack, ST_CPACK *apack, ST_PACK *pArrays);
...
// 参数  : ST_CPACK *apack[OUT]: 应答包
// 参数  : ST_PACK *pArrays[OUT]: 应答后续数据记录
BUPUB_API int IntCallNext(int waittime, ST_CPACK *apack, ST_PACK *pArrays);

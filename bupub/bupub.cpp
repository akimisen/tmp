...
* 修改日期: 2017-05-22
* 修改人员: 张晓川
* 修改描述: 新增 IntCall 与 IntCallNext
* 备注信息: SECREQ-5133 【东方】7*24小时服务项目-登陆功能日志的分离
* 版本信息: v5.0.3.0
* 专题信息: 东方7×24小时服务
------------------------------------------------------------------------- 
* 修改日期: 2018-02-28
* 修改人员: 陈永华
* 合并人员：曾会芳
* 修改描述: 为了能够支持多记录请求框架,不必要为多记录请求另外改写代码
* 备注信息: SECJZJY-25763【海通代码回归】2018004-20180324集中交易5031-h0（V5.0.3.1-h0）
* 版本信息: v5.0.4.0
* 专题信息: 海通回归(5026h7-5031h0)
*********************************************************************/
...
// 以下是专为集中交易而修改的  2008-12-26 12:54:35 
extern int g_retcode;
extern bool g_bFSetParmBits;
extern unsigned char ucFSetParmBits[32];

//////////////////////////////////////////////////////////////////////////
// 2018/2/28 星期三 18:12:41 为了能够支持多记录请求框架 by CYH  -- 不必要为多记录请求另外改写代码
// 对于多记录请求的应答，则将会以最后一条应答来返回，
// 若某个记录请求处理过程中，若符合(rtn==0 && *iRetCode!=-999 && *iRetCode!=NEED_CHECK)，也作为结束
//       后面的记录将不会被处理了，其后台数据库中的数据也因为rollback而回滚了。
bool g_bMultiReqPack = false;  // 本次处理是否为多记录请求模式
unsigned char  g_mrParmBits[32];  // MultiReqPack时候专用，保存最后的SetCol的ParmBits
ST_PACK g_mrPack;   // MultiReqPack时候专用，保存最后的PutRow的ST_PACK
int g_mrPutRows = 0;


void BeginMultiRPProcess()
{
   g_bMultiReqPack = true;
   g_mrPutRows = 0;
   memset(g_mrParmBits,0,sizeof(g_mrParmBits));
}
...
void EndMultiRPProcess(TRUSERID *handle,int *iRetCode, char *szMsg)
{
   if (g_bMultiReqPack)
   {
      g_bMultiReqPack = false;
      if (g_mrPutRows>0)
      {
         g_SvrLink.SetCol(handle, g_mrParmBits);
         PutRow(handle, &g_mrPack, iRetCode, szMsg);
      }
      g_mrPutRows = 0;
   }
}
...
*/
int SetCol(void *handle,...)
{
    va_list ap;
    int arg;
    ST_PACKHEAD head;
    memset(&head,0,sizeof(head));
    if (g_retcode==0 && g_bFSetParmBits)
    {
        memcpy(head.ParmBits,ucFSetParmBits,sizeof(head.ParmBits));
    }
    else
    {
        va_start(ap, handle);
    ...
    if (g_bMultiReqPack)
    {
        memcpy(g_mrParmBits, head.ParmBits, sizeof(g_mrParmBits));
        return(-1);
    }
    //////////////////////////////////////////////////////////////////////////
    g_SvrLink.SetCol((TRUSERID *)handle,head.ParmBits);
    return(-1);
}
...
{
    return(g_SvrLink.IntCall(szBUGroupID, waittime, rpack, apack, pArrays));
}
...
{
    return(g_SvrLink.IntCallNext(waittime, apack, pArrays));
}
...
// 返回  : int as BOOL : -1: TRUE; 0:FAIL   esp. for CTS
// 参数  : TRUSERID *handle
// 参数  : ST_PACK *pdata
// 参数  : int *pRetCode
// 参数  : char *rtnMsg
int PutRow(void *handle,ST_PACK *pdata,int *pRetCode,char *rtnMsg)
{
   // CYH + 2018/2/28 星期三 19:01:41 为了支持多记录请求：
   if (g_bMultiReqPack)
   {
      ++g_mrPutRows;
      memcpy(&g_mrPack, pdata, sizeof(ST_PACK));
      return(-1);
   }
   // 为了将返回模式修改为证券集中交易模式的
   if (g_SvrLink.PutRow((TRUSERID *)handle,pdata,*pRetCode,rtnMsg)!=0)
      return(0);
   else
      return(-1);
}

...
//int iThisProcID;
int g_retcode=0;   // 专门为了集中交易而作的变更
char g_omsg[2048];
bool g_bFSetParmBits=false;
unsigned char ucFSetParmBits[32];
...
int CSvrLink::Processing(ST_BUNIT *pBUnit)
{
   ...
   rp.iLastRetCode = htonl(g_retcode);
...
*/
   ...
   buflen += sizeof(RP_ANSDATAEXH);
   memcpy(sbuf+sizeof(RP_ANSDATAEXH),msgbuf,len);
   return(SendMsg(MT_ANSDATAEX,(unsigned int)buflen,sbuf));
}
   ...
   *******************************************************/
   ...
   if (pProcess!=NULL)
   {
      if (DecodeHead((unsigned char *)rdata,rdlen,&head,szMsg)==sizeof(ST_PACKHEAD))
      {
         ...
         g_omsg[0]='\0';
         g_retcode = 0;
         rtn = (*pProcess)(&ruserid,rdata,rdlen,&g_retcode,g_omsg);
         DataDone(g_retcode,g_omsg);
         return(rtn);
      }
   }

   // 2006-3-23 0:56:40 采用动态分配，以少占用堆栈空间
   pArrays = (ST_PACK *)malloc(sizeof(ST_PACK)*MAXROW);
   if (DecodeBufWithArray((unsigned char *)rdata,rdlen,&rpack,pArrays,&recs,szMsg))
   {
      int fno;
      ...
      // 为了JZJY的前台指定位图的需要
      g_bFSetParmBits = (strcmp((const char *)rpack.pack.usset6+32,"OutBitsSeted")==0);
      memcpy(ucFSetParmBits,rpack.pack.usset6,sizeof(ucFSetParmBits));
      //////////////////////////////////////////////////////////////////////////
...
#endif
         rtn = (*pCallBDFunc)(fno,&ruserid,&rpack,pArrays,&g_retcode,g_omsg);
         // 如果需要统计功能处理能力，在这里加上。。。
